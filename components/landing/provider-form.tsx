...
type ProviderFormProps = {
  /** When provided (modal usage), the success screen shows a "Cerrar" button.
   *  When omitted (page usage), it shows a link back to the home page. */
  onSuccessClose?: () => void;
  /** Autofocus the first field on mount (used by the modal). */
  autoFocus?: boolean;
};

export default function ProviderForm({ onSuccessClose, autoFocus = false }: ProviderFormProps) {
  ...
  useEffect(() => {
    if (autoFocus) firstFocusRef.current?.focus();
  }, [autoFocus]);
  ...
  if (submitState === "success") {
    return (
      <div className="al-provider-success">
        ...
        {onSuccessClose ? (
          <button className="al-provider-done-btn" onClick={onSuccessClose}>
            Cerrar
          </button>
        ) : (
          <Link className="al-provider-done-btn" href="/">
            Volver al inicio
          </Link>
        )}
      </div>
