...
- [ ] T008 [US1] En `components/forms/provider-form.tsx`, enganchar `google.maps.places.Autocomplete` al input `prov-address` con `componentRestrictions: { country: "ar" }` y `types: ["address"]` (no `"(regions)"`), extrayendo el geocode con `findAddressComponent` de `@/lib/google-places` (depende de T007)
...
- [ ] T010 [US1] Agregar los campos de horarios (texto libre) y modalidad (`en_local`/`a_domicilio`/`ambas`) al formulario en `components/forms/provider-form.tsx`, reusando los primitivos `Field`/`Input`/`ChoiceRow` ya usados en el resto del form; ambos opcionales, sin bloquear el envío (research.md, Decisión 5)
